  ...
  if("serviceWorker" in navigator) {
    ...
    window.addEventListener("load", (event) => {
      ...
      const registration = navigator.serviceWorker.register("sw.js").then((registration) => {
        ...
        console.log("ServiceWorker reg successfull:", registration);
      }).catch((error) => {
        // B5. TODO - In the event that the service worker registration fails, console
        //            log that it has failed.
        console.log(`ServiceWork reg failed: ${error}`);
      });
      // STEPS B6 ONWARDS WILL BE IN /sw.js
    })
  }
  else {
    console.log("ServiceWork not supported by current browser!")
  }
}
...
async function getRecipes() {
  ...
  return myPromise = new Promise(async (resolve, reject) => {
      /**************************/
      ...
      //            declared above
      const  urlLen = RECIPE_URLS.length;
      for(let i = 0; i < urlLen; i++) {
        ...
        try {
          ...
          //            "await" again
          let recipeJSON = await fetchResponse.json();
